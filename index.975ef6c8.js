...
refs.formBox.addEventListener("input", (0, _lodashDebounceDefault.default)(search, DEBOUNCE_DELAY));
console.log(refs.formBox);
function search(e) {
    e.preventDefault();
    const nameCountry = refs.formBox.value.trim();
    clearContainer();
    if (nameCountry) (0, _fetchCountries.fetchСountries)(nameCountry).then((data)=>searchCountry(data));
}
//  Создаем функцию поиска  //
function searchCountry(countries) {
    if (countries.length > 10) (0, _notiflixNotifyAio.Notify).success("Too many matches found. Please enter a more specific name.");
    else if (countries.length >= 2 && countries.length < 10) renderCountryList(countries);
    else if (countries.length === 1) renderCountryCard(countries);
    else clearError(countries);
}
function renderCountryCard(countries) {
    const cardMarkup = countries.map(({ name , capital , population , flags , languages  })=>{
...
function clearError(countries) {
    if (countries.length === 0) throw new Error("404");
    else (0, _notiflixNotifyAio.Notify).failure("Oops, there is no country with that name");
}
...
},{"./css/styles.css":"1CY4s","./fetchCountries":"fTQqv","lodash.debounce":"3JP5n","notiflix/build/notiflix-notify-aio":"eXQLZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"fTQqv":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "fetch\u0421ountries", ()=>fetchСountries);
const BASE_URL = `https://restcountries.com/v2/`;
function fetchСountries(name) {
    return fetch(`${BASE_URL}/name/${name}?fields=name,capital,population,languages,flags`).then((response)=>{
